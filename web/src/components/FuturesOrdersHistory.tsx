...
export default function FuturesOrdersHistory() {
  ...
  useEffect(() => {
    // Handler for WebSocket events - triggers immediate refresh
    const handleOrderEvent = () => {
      // Always refresh orders on any order-related event for instant UI update
      if (activeTab === 'orders') {
        fetchOrders();
      }
    };
    ...
    // Handler for position updates - when position closes, orders should update
    const handlePositionUpdate = () => {
      // When a position closes, associated TP/SL orders may be removed
      // Refresh orders immediately to reflect changes
      if (activeTab === 'orders') {
        fetchOrders();
      }
      // Also refresh trade history as position close creates a trade
      if (activeTab === 'history') {
        fetchTrades();
      }
    };
