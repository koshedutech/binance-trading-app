...
func TestModeStrategySettingsRow(t *testing.T) {
	...
	if row.UserID != 123 {
		t.Errorf("Expected UserID=123, got %d", row.UserID)
	}
	if row.Mode != "scalp" {
...
func (m *MockModeStrategyRepository) key(userID int, mode, strategy string) string {
	return string(rune(userID)) + ":" + mode + ":" + strategy
}

func (m *MockModeStrategyRepository) keyInt(userID int, mode, strategy string) string {
	return fmt.Sprintf("%d:%s:%s", userID, mode, strategy)
}

// CreateModeStrategySettings creates a new mode+strategy configuration.
func (m *MockModeStrategyRepository) CreateModeStrategySettings(ctx context.Context, userID int, mode, strategy string, enabled bool, priority int, settings json.RawMessage) error {
	m.mu.Lock()
	defer m.mu.Unlock()

	m.seq++
	key := m.keyInt(userID, mode, strategy)
...
func (m *MockModeStrategyRepository) GetModeStrategySettings(ctx context.Context, userID int, mode, strategy string) (*ModeStrategySettingsRow, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *MockModeStrategyRepository) GetAllModeStrategySettings(ctx context.Context, userID int) ([]*ModeStrategySettingsRow, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *MockModeStrategyRepository) GetModeStrategies(ctx context.Context, userID int, mode string) ([]*ModeStrategySettingsRow, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()

	var result []*ModeStrategySettingsRow
	prefix := fmt.Sprintf("%d:%s:", userID, mode)
...
func (m *MockModeStrategyRepository) UpdateModeStrategySettings(ctx context.Context, userID int, mode, strategy string, settings json.RawMessage) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockModeStrategyRepository) UpdateModeStrategyEnabled(ctx context.Context, userID int, mode, strategy string, enabled bool) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockModeStrategyRepository) DeleteModeStrategySettings(ctx context.Context, userID int, mode, strategy string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockModeStrategyRepository) BulkCreateModeStrategySettings(ctx context.Context, userID int, configs []ModeStrategySettingsRow) error {
	m.mu.Lock()
	defer m.mu.Unlock()

	for _, config := range configs {
		m.seq++
		key := m.keyInt(userID, config.Mode, config.Strategy)
