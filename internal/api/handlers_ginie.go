...
func (s *Server) handleToggleGinie(c *gin.Context) {
	// Get the per-user GinieAutopilot instance (multi-user mode)
	giniePilot := s.getGinieAutopilotForUser(c)
	if giniePilot == nil {
	...
	if req.Enabled {
		// Start the per-user Ginie autopilot
		if err := giniePilot.Start(); err != nil {
			// Check if it's already running - that's OK, not an error
			if giniePilot.IsRunning() {
				log.Printf("[GINIE-TOGGLE] Ginie already running for user, continuing")
			} else {
		...
		// Stop the per-user Ginie autopilot
		// CRITICAL FIX: Check error from Stop() - log but don't fail if already stopped
		if err := giniePilot.Stop(); err != nil {
			// "not running" is OK when disabling, just log it
			log.Printf("[GINIE-TOGGLE] Stop returned: %v (this is OK if already stopped)", err)
		}
	}

	enabled := giniePilot.IsRunning()

	// CRITICAL FIX: Persist the auto-start setting so Ginie restarts after server reboot
	// This was missing before, causing Ginie to not auto-start on server restart
	...
	// Get current dry_run mode from the autopilot config
	dryRun := giniePilot.GetConfig().DryRun

	// Broadcast autopilot status change to the SPECIFIC user via WebSocket (multi-user safe)
	userID := s.getUserID(c)
	if userWSHub != nil && userID != "" {
