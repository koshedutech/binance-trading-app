...
// ============================================================================
// MODE STRATEGY METHODS (Story 11.30)
// ============================================================================
...
func (ts *TestableSettingsCacheService) GetModeStrategyConfig(ctx context.Context, userID int, mode, strategy string) (*database.ModeStrategyConfig, error) {
	...
	ts.mockCache.Set(ctx, key, string(data), ModeStrategyCacheTTL)
	return config, nil
}
...
func (ts *TestableSettingsCacheService) GetAllStrategiesForMode(ctx context.Context, userID int, mode string) (map[string]*database.ModeStrategyConfig, error) {
	...
	// Try to get all strategies from cache
...
func (ts *TestableSettingsCacheService) PopulateModeStrategiesFromDB(ctx context.Context, userID int) error {
	...
	// Populate all mode+strategy combinations with defaults
	for _, mode := range TradingModes {
		for _, strategy := range ValidStrategies {
			config := database.DefaultModeStrategyConfig(mode, strategy)
			key := modeStrategyKey(userID, mode, strategy)
...
// ============================================================================
// MODE STRATEGY CACHE TESTS
// Epic 11: Position Decision Engine - Story 11.30: Mode-Strategy Cache Layer
// ============================================================================
...
// TestGetModeStrategyConfig_CacheHit tests cache hit scenario
func TestGetModeStrategyConfig_CacheHit(t *testing.T) {
	ts := NewTestableSettingsCacheService()
	ctx := context.Background()
	...
	// Setup: Data in cache
	cacheKey := "mode:123:scalp:trend_following"
	config := &database.ModeStrategyConfig{
		Enabled:  true,
		Priority: 1,
		Leverage: 10,
	}
...
// TestGetModeStrategyConfig_CacheUnavailable tests error when cache is down
func TestGetModeStrategyConfig_CacheUnavailable(t *testing.T) {
	ts := NewTestableSettingsCacheService()
	ctx := context.Background()
...
// TestSetModeStrategyConfig_WriteThrough tests write-through caching
func TestSetModeStrategyConfig_WriteThrough(t *testing.T) {
	ts := NewTestableSettingsCacheService()
	ctx := context.Background()
	...
	// Assert: Cache was updated
	cacheKey := "mode:123:scalp:trend_following"
	if _, exists := ts.mockCache.data[cacheKey]; !exists {
		t.Error("Expected cache to be updated")
	}

	// Assert: Cache Set was called
	if len(ts.mockCache.setCalls) < 1 {
		t.Errorf("Expected cache Set call")
	}
}

// TestInvalidateModeStrategyConfig tests cache invalidation
func TestInvalidateModeStrategyConfig(t *testing.T) {
	ts := NewTestableSettingsCacheService()
	ctx := context.Background()
	...
	if len(ts.mockCache.deleteCalls) != 1 {
		t.Errorf("Expected 1 delete call, got %d", len(ts.mockCache.deleteCalls))
	}
	if ts.mockCache.deleteCalls[0] != cacheKey {
		t.Errorf("Expected delete key %s, got %s", cacheKey, ts.mockCache.deleteCalls[0])
	}
}

// TestInvalidateAllModeStrategies tests pattern-based cache invalidation
func TestInvalidateAllModeStrategies(t *testing.T) {
	ts := NewTestableSettingsCacheService()
	ctx := context.Background()
	...
	// Assert: DeletePattern was called
	if len(ts.mockCache.patternCalls) != 1 {
		t.Errorf("Expected 1 pattern delete call, got %d", len(ts.mockCache.patternCalls))
	}
	expectedPattern := "mode:123:*"
	if ts.mockCache.patternCalls[0] != expectedPattern {
		t.Errorf("Expected pattern %s, got %s", expectedPattern, ts.mockCache.patternCalls[0])
	}
}

// TestGetAllStrategiesForMode_AllFromCache tests getting all strategies from cache
func TestGetAllStrategiesForMode_AllFromCache(t *testing.T) {
	ts := NewTestableSettingsCacheService()
	ctx := context.Background()
	...
	// Setup: All 4 strategies in cache
	for _, strategy := range ValidStrategies {
		cacheKey := fmt.Sprintf("mode:123:scalp:%s", strategy)
		config := &database.ModeStrategyConfig{
			Enabled:  strategy == "trend_following",
			Priority: 1,
			Leverage: 10,
		}
		data, _ := json.Marshal(config)
	...
	// Assert: Got all 4 strategies
	if len(result) != 4 {
		t.Errorf("Expected 4 strategies, got %d", len(result))
	}

	// Assert: trend_following is enabled
	if config, ok := result["trend_following"]; ok {
		if !config.Enabled {
			t.Error("Expected trend_following to be enabled")
		}
	} else {
		t.Error("Expected trend_following in result")
	}
}

// TestGetAllStrategiesForMode_CacheUnavailable tests error when cache is down
func TestGetAllStrategiesForMode_CacheUnavailable(t *testing.T) {
	ts := NewTestableSettingsCacheService()
	ctx := context.Background()
...
// TestPopulateModeStrategiesFromDB_CacheUnavailable tests error when cache is down
func TestPopulateModeStrategiesFromDB_CacheUnavailable(t *testing.T) {
	ts := NewTestableSettingsCacheService()
	ctx := context.Background()
...
// TestModeStrategyCacheTTL verifies the TTL constant is set correctly
func TestModeStrategyCacheTTL(t *testing.T) {
	expectedTTL := 24 * time.Hour
	if ModeStrategyCacheTTL != expectedTTL {
		t.Errorf("Expected ModeStrategyCacheTTL=%v, got %v", expectedTTL, ModeStrategyCacheTTL)
	}
}

// TestValidStrategies verifies all valid strategies are defined
func TestValidStrategies(t *testing.T) {
	expectedStrategies := []string{"trend_following", "mean_reversion", "breakout", "range_trading"}

	if len(ValidStrategies) != len(expectedStrategies) {
		t.Errorf("Expected %d strategies, got %d", len(expectedStrategies), len(ValidStrategies))
	}

	for i, expected := range expectedStrategies {
		if ValidStrategies[i] != expected {
			t.Errorf("Expected strategy[%d]=%s, got %s", i, expected, ValidStrategies[i])
		}
	}
}
